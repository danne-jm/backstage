...
export default function ShopCart({ sellables, processingFeeRate }: Props) {
    ...
    const [email, setEmail] = useState(() => {
        if (typeof window !== 'undefined') return localStorage.getItem('cart_email') || '';
        return '';
    });
        ...
        return entries
            .map((entry): CartItem | null => {
                ...
                return {
                    ...
                    member_price: sellable.member_price ?? undefined,
                };
            })
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            try {
                ...
                setServerWarnings(Array.isArray(res.data.warnings) ? res.data.warnings : []);
            } catch (e) {
    ...
    const cartWarnings = useMemo(() => {
        ...
        cart.forEach((item) => {
            ...
            if (sellable.is_variant_based && sellable.variants.length > 0) {
                ...
                const isDiscounted = discountApplied && item.member_price && item.member_price < item.price;
                if (isDiscounted && !sellable.has_stock_with_card) {
    ...
    const handleApplyDiscount = async () => {
        ...
        if (appliedDiscounts.includes(code)) {
            setMessage({ text: 'This code is already applied.', type: 'error' });
            return;
        }
        ...
        try {
            ...
            if (validCodes.includes(code)) {
                ...
                setMessage({ text: 'Invalid or expired ESNcard code.', type: 'error' });
            }
        } catch (e) {
            console.error(e);
            setMessage({ text: 'Failed to validate code. Please try again.', type: 'error' });
        } finally {
    ...
    const removeDiscount = (codeToRemove: string) => {
        const newDiscounts = appliedDiscounts.filter((code) => code !== codeToRemove);
        setAppliedDiscounts(newDiscounts);
        localStorage.setItem('cart_discounts', JSON.stringify(newDiscounts));
    };
    ...
    const handleCheckout = async () => {
        ...
        try {
            const items = cart.map((item) => ({
                ...
                use_member_price: discountApplied && item.member_price && item.member_price < item.price,
            }));
            ...
            <div className="bg-white">
                <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-16 lg:px-8">
                    ...
                    <div className="mt-8 lg:mt-12 lg:grid lg:grid-cols-12 lg:items-start lg:gap-x-12 xl:gap-x-16">
                                            ...
                                            <div className="relative pr-8 sm:grid sm:grid-cols-2 sm:gap-x-4 sm:pr-0 lg:gap-x-6">
                                                ...
                                                <div className="mt-3 sm:mt-0 sm:pr-9">
                                                    <div className="flex h-8 w-24 items-center border border-gray-300 sm:h-10 sm:w-32">
                                                        <button
                                                            onClick={() => updateQuantity(item.id, item.type, item.quantity - 1, item.options)}
                                                    ...
                                                    <div className="absolute top-0 right-0">
                                                        <button
                                                            type="button"
                                                            onClick={() => removeFromCart(item.id, item.type, item.options)}
                        ...
                        >
                            <h2 id="summary-heading" className="text-base font-medium text-gray-900 sm:text-lg">
                                Order summary
                            </h2>
                            ...
                            <dl className="mt-4 space-y-3 sm:mt-6 sm:space-y-4">
                                ...
                                <div className="border-t border-gray-200 pt-3 sm:pt-4">
                                    ...
                                    >
                                        ...
                                        {isDiscountOpen ? (
                                            <ChevronUp className="h-4 w-4 text-gray-500 sm:h-5 sm:w-5" />
                                        ) : (
                                            ...
                                            <div className="mt-3 pb-1 sm:mt-4">
                                                        ...
                                                        {appliedDiscounts.map((code) => (
                                                            ...
                                                            >
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="ml-1 inline-flex h-3.5 w-3.5 flex-shrink-0 items-center justify-center rounded-full text-gray-400 hover:bg-gray-200 hover:text-gray-500 focus:outline-none sm:ml-1.5 sm:h-4 sm:w-4"
                                                                >
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        className="flex-shrink-0 rounded-md bg-gray-200 px-3 py-2 text-xs font-medium text-gray-900 hover:bg-gray-300 disabled:opacity-50 sm:px-4 sm:text-sm"
                                                    >
                                                        Apply
                                                    </button>
                                                </div>
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <dd className="text-sm font-medium text-gray-900">
                                        +€{(Number(serverTotal ?? originalTotal) * Number(processingFeeRate)).toFixed(2)}
                                    </dd>
                                </div>

                                <div className="flex items-center justify-between border-t border-gray-200 pt-3 sm:pt-4">
                                    <dt className="text-sm font-medium text-gray-900 sm:text-base">Order total</dt>
                                    <dd className="text-xl font-bold text-gray-900 sm:text-2xl">
                                        €{(Number(serverTotal ?? originalTotal) * (1 + Number(processingFeeRate))).toFixed(2)}
                                    </dd>
                                </div>
                            </dl>
                            ...
                            <div className="mt-4 border-t border-gray-200 pt-4 sm:mt-6 sm:pt-6">
                                ...
                                <p className="mt-1 text-xs text-gray-500">Your order confirmation will be sent here</p>
                                ...
                                {emailError && <p className="mt-1 text-xs text-red-600 sm:text-sm">{emailError}</p>}
                            </div>
                            ...
                            {cartWarnings.length > 0 && (
                                <div className="mt-3 rounded-md bg-yellow-50 p-2.5 text-sm text-yellow-700 sm:mt-4 sm:p-3">
                                    ...
                                    <ul className="list-disc pl-5 text-xs sm:text-sm">
                                        {cartWarnings.map((w, i) => <li key={i}>{w}</li>)}
                                    </ul>
                                </div>
                            )}
