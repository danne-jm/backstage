...
export const useOfficeShift = () => {
    ...
    const { mutate } = useSWR((initialProps.activeShift as any) ? `/office/${(initialProps.activeShift as any).id}` : null, fetcher, {
    ...
    const staffMap = React.useMemo(
        () => new Map(((props['staff'] as any[]) || []).map((s: any) => [s.email, s.name])),
        [props['staff']],
    );
