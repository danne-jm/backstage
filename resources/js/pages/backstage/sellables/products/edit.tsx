...
export default function EditProduct({
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        start_sell_date: product.start_sell_date
            ? product.start_sell_date.slice(0, 16)
            : '',
        end_sell_date: product.end_sell_date
            ? product.end_sell_date.slice(0, 16)
            : '',
        price_without_membership:
            product.price_without_membership?.toString() || '',
        ...
        unlimited_quantity_with_membership:
            product.unlimited_quantity_with_membership ?? false,
        quantity_with_membership:
            product.quantity_with_membership?.toString() || '',
        unlimited_quantity_without_membership:
            product.unlimited_quantity_without_membership ?? false,
        quantity_without_membership:
            product.quantity_without_membership?.toString() || '',
    ...
    const [pricingType, setPricingType] = useState<'single' | 'split'>(
        product.price_without_membership !== product.price_with_membership
            ? 'split'
            : 'single',
    );
    const [stockType, setStockType] = useState<'simple' | 'split' | 'variants'>(
        product.is_variant_based
            ? 'variants'
            : product.quantity_with_membership !== null ||
                product.unlimited_quantity_with_membership
              ? 'split'
              : 'simple',
    );
    ...
    useEffect(() => {
        if (product.is_variant_based && product.variants_config?.length > 0) {
            const keys = Object.keys(product.variants_config[0]).filter(
                (k) => k !== 'id' && k !== 'quantity',
            );
            const attrs: Attribute[] = keys.map((k) => {
                const options = Array.from(
                    new Set(
                        product.variants_config.map((r: any) =>
                            String(r[k] ?? ''),
                        ),
                    ),
                );
    ...
    const submit = (e: React.FormEvent) => {
        ...
        router.post(
            updateProductRoute(product.id).url,
            {
                ...data,
                _method: 'PUT',
