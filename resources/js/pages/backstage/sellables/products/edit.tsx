...
import { update as updateProductRoute } from '@/routes/backstage/sellables/products';
...
export default function EditProduct({ users, membershipCardName, product }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        name: product.name || '',
        description: product.description || '',
        price: product.price?.toString() || '',
        start_sell_date: product.start_sell_date ? product.start_sell_date.slice(0, 16) : '',
        end_sell_date: product.end_sell_date ? product.end_sell_date.slice(0, 16) : '',
        price_without_membership: product.price_without_membership?.toString() || '',
        price_with_membership: product.price_with_membership?.toString() || '',
        is_online_sellable: product.is_online_sellable ?? true,
        hide_until_sale: product.hide_until_sale ?? false,
        unlimited_quantity: product.unlimited_quantity ?? false,
        quantity: product.quantity?.toString() || '',
        unlimited_quantity_with_membership: product.unlimited_quantity_with_membership ?? false,
        quantity_with_membership: product.quantity_with_membership?.toString() || '',
        unlimited_quantity_without_membership: product.unlimited_quantity_without_membership ?? false,
        quantity_without_membership: product.quantity_without_membership?.toString() || '',
        is_variant_based: product.is_variant_based ?? false,
        variants_config: product.variants_config || null as any,
        instagram_link: product.instagram_link || '',
        responsible_user_ids: product.responsible_user_ids || [] as string[],
        image: null as File | null,
        remove_image: false,
    });

    const [pricingType, setPricingType] = useState<'single' | 'split'>(
        product.price_without_membership !== product.price_with_membership ? 'split' : 'single'
    );
    const [stockType, setStockType] = useState<'simple' | 'split' | 'variants'>(
        product.is_variant_based ? 'variants' : (product.quantity_with_membership !== null || product.unlimited_quantity_with_membership ? 'split' : 'simple')
    );
    ...
    const [previewUrl, setPreviewUrl] = useState<string | null>(product.image_path || null);
    ...
    useEffect(() => {
        if (product.is_variant_based && product.variants_config?.length > 0) {
            const keys = Object.keys(product.variants_config[0]).filter(k => k !== 'id' && k !== 'quantity');
            const attrs: Attribute[] = keys.map(k => {
                const options = Array.from(new Set(product.variants_config.map((r: any) => String(r[k] ?? ''))));

                return { id: crypto.randomUUID(), name: k, options: options as string[] };
            });
            setAttributes(attrs);
            setVariantRows(product.variants_config.map((r: any) => {
...
cfg[k] = row[k];
        ...
        router.post(updateProductRoute(product.id).url, {
            ...data,
            _method: 'PUT',
            price: Number(data.price_with_membership || 0),
                ...
                <div className="w-full p-6 max-w-5xl mx-auto">
                    ...
                    <div className="flex items-center gap-3 mb-6">
                        <div>
                            ...
                            <p className="text-sm text-zinc-500">Update the details for the product.</p>
                        </div>
                    </div>
