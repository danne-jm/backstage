                ...
                <div className="flex flex-wrap gap-1.5">
                    {filteredSuggestions.map((s) => (
                        <button
                            ...
                            type="button"
                            onClick={() => addTag(s)}
                            className="rounded-md bg-zinc-800/60 px-2.5 py-0.5 text-xs font-medium text-zinc-400 transition-colors hover:bg-zinc-700 hover:text-zinc-200"
                        >
                            {s}
                        </button>
                    ))}
...
export default function Inventory({
    ...
    const filteredItems = items.filter((item) => {
        ...
        return (
            ...
            (item.category &&
                item.category.join(' ').toLowerCase().includes(query))
        );
    });
                ...
                <div className="rounded-md border bg-zinc-950">
                    <div className="w-full overflow-auto">
                        <table className="w-full text-left text-sm">
                            <thead className="border-b bg-zinc-900/50 text-xs text-muted-foreground uppercase">
                                <tr>
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-4">
                                            {item.image_path ? (
                                                ...
                                                <div className="flex h-10 w-10 items-center justify-center rounded bg-zinc-800 text-center text-xs text-zinc-500">
                                                    No IMG
                                                </div>
                                            )}
                                        </td>
                                        <td className="px-4 py-4">
                                            {item.category?.map((cat) => (
                                                <span
                                                    key={cat}
                                                    className="mr-1 inline-flex items-center rounded-md bg-zinc-800 px-2.5 py-0.5 text-xs font-medium text-zinc-300"
                                                >
                                                    {cat}
                                                </span>
                                            ))}
