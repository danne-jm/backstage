    ...
    React.useEffect(() => {
        const interval = setInterval(() => {
            router.get(office().url, {}, {
                ...
                preserveScroll: true,
                replace: true,
                only: ['activeShift', 'lastShift', 'products', 'pastShifts'],
            });
        }, 2000);
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="grid gap-4 md:grid-cols-3 md:items-stretch">
                    <section className="rounded-xl border border-sidebar-border/70 bg-background p-4 dark:border-sidebar-border flex flex-col h-full">
                    ...
                    <section className="rounded-xl border border-sidebar-border/70 bg-background p-4 dark:border-sidebar-border flex flex-col h-full">
                        <h3 className="mb-3 text-sm font-semibold">Sellable Products</h3>
                ...
                <div className="relative flex-1 overflow-hidden rounded-xl border border-sidebar-border/70 bg-background p-4 dark:border-sidebar-border">
                    <h3 className="mb-4 text-sm font-semibold">Previous Shift Sales Log</h3>
                    ...
                    {lastShift && Array.isArray(lastShift.sales) && lastShift.sales.length > 0 ? (
                                ...
                                <tbody className="mt-2">
                                    {lastShift.sales.map((sale: any) => (
                                        <tr key={String(sale.id)} className="border-t">
                                            ...
                                            <td className="py-3">{sale.name ?? 'N/A'}</td>
                                            <td className="py-3 capitalize">{sale.method}</td>
                                            <td className="py-3">€{Number(sale.amount ?? 0).toFixed(2)}</td>
                                            <td className="py-3">{sale.description ?? ''}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ...
                        <div className="text-sm text-muted-foreground">No sales data from previous shift</div>
                    )}
                </div>
