...
import { store } from '@backstage/routes/login';
import { login as googleLogin } from '@backstage/routes/google';
...
export default function Login({ status }: Props) {
    ...
    function handleGoogleLogin() {
        window.location.href = googleLogin.url();
    }
        ...
        >
            ...
            {(status || flash?.error) && (
                <div className="mt-2 text-center text-sm font-medium">
                    ...
                    {flash?.error && <span className="text-destructive">{flash.error}</span>}
                </div>
            )}
        </AuthLayout>
    );
