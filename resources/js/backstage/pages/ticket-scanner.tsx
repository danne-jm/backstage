import { AttendeeDetailsDialog } from '@backstage/components/ticket-scanner/attendee-details-dialog';
import { AvailableAttendeesList } from '@backstage/components/ticket-scanner/available-attendees-list';
import { ScanResultDialog } from '@backstage/components/ticket-scanner/scan-result-dialog';
import { ScannedTicketDialog } from '@backstage/components/ticket-scanner/scanned-ticket-dialog';
import { ScannedTicketsList } from '@backstage/components/ticket-scanner/scanned-tickets-list';
...
export default function TicketScanner() {
    ...
    const filteredEvents = React.useMemo(() => {
        ...
        return events.filter((ev) => {
            ...
            // Relax constraint: show future events too, or just recent past + future
            // Original: d >= now && d <= end. "now" includes time. So events earlier today might be filtered out if time is checked.
            // Let's just use start of day for 'now'.
    ...
    React.useEffect(() => {
        if (!selectedEvent || !(window as any).Echo) return;

        const channel = (window as any).Echo.private(`tickets.${selectedEvent}`);
        ...
        return () => {
            (window as any).Echo?.leave(`tickets.${selectedEvent}`);
        };
    }, [selectedEvent]);
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.valid && data.ticket) {
                    ...
                    if (selectedEvent) {
                         // Optimistic update locally first? Or wait for Reverb?
                         // Reverb handles it. But let's reload anyway to be safe/synced.
                         // Actually, removing reload to rely on Reverb might be faster, but let's keep reload for reliability if Reverb fails.
                        const availableRes = await fetch(
                            `/ticket-scanner/available-tickets?event_id=${selectedEvent}`,
                            {
                                headers: {
                ...
                setLastScan(
                    `✗ ${errData.error || errData.message || 'Unknown ticket'}`,
                );
            ...
            await scanner.start(
                ...
                async (decodedText: string) => {
                    ...
                    if (
                        ...
                        // Stop camera after successful scan? The modal will open and stop camera via effect.
                    }
                },
                (errorMessage: string) => {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="p-2 sm:p-4 md:p-6">
                ...
                <AttendeeDetailsDialog
                    ...
                    onVerify={(code) => void verifyTicket(code)}
                />
