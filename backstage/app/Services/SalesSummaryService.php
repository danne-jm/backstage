...
{
    public function generateSummary(Request $request): array
    {
        [$start, $end, $hourly] = $this->calculateDateRange($request);

        $cacheKey = 'sales_summary_'.md5(json_encode([$start->toString(), $end->toString(), $hourly]));

        return Cache::remember($cacheKey, 30, function () use ($start, $end, $hourly) {
            if ($hourly) {
                return $this->getHourlySummary($start, $end);
            }

            return $this->getDailySummary($start, $end);
        });
    }

    private function getHourlySummary(Carbon $start, Carbon $end): array
    {
    ...
    private function getDailySummary(Carbon $start, Carbon $end): array
    {
